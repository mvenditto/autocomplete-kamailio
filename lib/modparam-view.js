  ...
  render () {
    ...
    for (var mod in this.modparams){
        ...
        let params = this.modparams[mod].params.map(function(p) {
            let type = self.getValueTypeClass(p.value)
            return <li class='list-item' on={{click: () => self.modparamSelected(p)}}>
                        <span class='icon icon-settings'>{p.param} = </span>
                        <span class={type}> {p.value}</span>
                    </li>
        })
        modparamsTree.push(<li class='list-nested-item hover-item collapsed'>
