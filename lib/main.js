...
function findModparams() {
    ...
    while ((r = modparamRegex.exec(text)) !== null) {
        ...
        modparams[module].params.push({
            pos: buff.positionForCharacterIndex(r.index),
            param: r[2],
            value: r[3]
        })
    }
...
function onRouteItemClick(route) {
  ...
  editor.setCursorBufferPosition(route.pos, {autscroll: true})
...
export function activate() {
    ...
    const modparamsView = new ModparamsView({
        modparams: [],
        modparamSelected: onRouteItemClick
    })
